...
exports.getByUsuarioId = getByUsuarioId;
exports.Post = Post;
exports.Put = Put;
...
async function getAll(req, res) {
    try {
        ...
        const ventas = await ventas_1.default.findAll({
            where,
            include: [{
...
async function getByUsuarioId(req, res) {
    try {
...
async function Post(req, res) {
    let transaction;
    try {
        const { usuario_id, nombre_cliente, metodo_pago, notas, carrito } = req.body;
        // ✅ VALIDACIONES ANTES DE CREAR LA TRANSACCIÓN
        if (!validator_1.Validator.isNotEmpty(nombre_cliente)) {
            return res.status(400).json({ error: 'El nombre del cliente es obligatorio' });
        }
        if (!validator_1.Validator.isNotEmpty(metodo_pago)) {
        ...
        const productos = await products_1.default.findAll({
        ...
        // Crear la venta
        const venta = await ventas_1.default.create({
        ...
        await detalleventa_1.default.bulkCreate(detallesConVentaId, { transaction });
        // ✅ COMMIT EXITOSO
        ...
        try {
            const ventaCompleta = await ventas_1.default.findByPk(venta.id, {
                include: [{
                        model: detalleventa_1.default,
                        as: 'detalle_venta'
                    }]
...
async function Put(req, res) {
    try {
        const venta = await ventas_1.default.findByPk(req.params.id);
        if (!venta)
            return res.sendStatus(404);
        const { estado, notas, metodo_pago } = req.body;
        if (estado) {
            const estadosValidos = ['completada', 'cancelada', 'pendiente', 'espera', 'aprobada'];
...
async function deleteById(req, res) {
    try {
        const n = await ventas_1.default.destroy({ where: { id: req.params.id } });
